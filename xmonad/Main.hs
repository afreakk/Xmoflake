...
myCmds cfg conf =
  [ ("default-layout", setLayout $ XM.layoutHook conf),
    ...
    ("notif-history-pop", spawn "qs ipc -c system-popups call notifications historyPop"),
...
manipulateFloatLabel = "ManipulateFloat"

manipulateFloat :: MDL.Mode
manipulateFloat =
  MDL.mode manipulateFloatLabel $ \XConfig {modMask = modm} ->
    M.fromList
...
myKeys cfg conf@XConfig {XM.modMask = modm} =
  M.fromList $
    [ ((modm .|. shiftMask, xK_Return), spawn $ XM.terminal conf),
      ...
      ((modm .|. shiftMask, xK_f), MDL.setMode manipulateFloatLabel),
...
myMouseBindings XConfig {XM.modMask = modm} =
  M.fromList
...
applyRefocusLastHooks c =
  c
    { handleEventHook = handleEventHook c <+> refocusLastWhen isFloat,
